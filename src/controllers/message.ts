...
class MessageRouter {
  ...
  @tag
  static async getMessages(ctx: Koa.ParameterizedContext<Koa.DefaultState, Koa.DefaultContext>, next: Koa.Next) {
  ...
  @tag
  static async getMessageCount(ctx: Koa.ParameterizedContext<Koa.DefaultState, Koa.DefaultContext>, next: Koa.Next) {
    let user = ctx.state.user.data;
  ...
  @summary("把消息全部标记为已读")
  @tag
  static async markMessageRead(ctx: Koa.ParameterizedContext<Koa.DefaultState, Koa.DefaultContext>, next: Koa.Next) {
    let user = ctx.state.user.data;
  ...
  @tag
  static async clearMessages(ctx: Koa.ParameterizedContext<Koa.DefaultState, Koa.DefaultContext>, next: Koa.Next) {
    let page = ctx.query.page || 0;
    let size = ctx.query.size || 10;
    let user = ctx.state.user.data;

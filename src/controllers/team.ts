...
class TeamRouter {
    ...
    static async createTeam(ctx: Koa.ParameterizedContext<Koa.DefaultState, Koa.DefaultContext>, next: Koa.Next) {
        var user = ctx.state.user.data;
        var { body } = ctx.request;
        var team: any = new Team(body);
    ...
    static async dissolveTeam(ctx: Koa.ParameterizedContext<Koa.DefaultState, Koa.DefaultContext>, next: Koa.Next) {
        const { id } = ctx.validatedParams;
        var user = ctx.state.user.data;
        var team: any = await Team.findOne({ '_id': id, 'members.username': user.username, 'members.role': 'owner' });
        if (!team) {
    ...
    static async changeMemberRole(ctx: Koa.ParameterizedContext<Koa.DefaultState, Koa.DefaultContext>, next: Koa.Next) {
        var { teamId } = ctx.validatedParams;
        var user = ctx.state.user.data;
        var body = ctx.request.body
        var team = userInTeamIsManager(user._id, teamId)
        if (!team) {
    ...
    static async addMember(ctx: Koa.ParameterizedContext<Koa.DefaultState, Koa.DefaultContext>, next: Koa.Next) {
        var { teamId } = ctx.validatedParams;
        var user = ctx.state.user.data;
        ...
        var team: any = await Team.findOne({
            _id: teamId,
            members: {
                $elemMatch: {
                    _id: user._id,
        ...
        var userList: any = await User.find({
            email: { $in: emailList }
        }, "username email")
    ...
    static async removeMember(ctx: Koa.ParameterizedContext<Koa.DefaultState, Koa.DefaultContext>, next: Koa.Next) {
    ...
    static async getMembers(ctx: Koa.ParameterizedContext<Koa.DefaultState, Koa.DefaultContext>, next: Koa.Next) {
        var { teamId } = ctx.validatedParams;
        var user = ctx.state.user.data;
        //如果传入的id和当前登录用户的id相等 表示是自己离开团队
    ...
    static async updateTeamProfile(ctx: Koa.ParameterizedContext<Koa.DefaultState, Koa.DefaultContext>, next: Koa.Next) {
        console.log(ctx)
        var { teamId } = ctx.validatedParams;
        var user = ctx.state.user.data;
        var body = ctx.request.body
